...
def send_telegram(message):
    ...
    try:
        req.post(url, data={"chat_id": TELEGRAM_CHAT_ID, "text": message, "parse_mode": "HTML"}, timeout=5)
    except:
        pass

def get_config():
    return {"name": RESTAURANT_NAME, "slogan": RESTAURANT_SLOGAN,
            "hero": RESTAURANT_HERO, "desc": RESTAURANT_DESC,
            "primary": PRIMARY_COLOR, "secondary": SECONDARY_COLOR, "dev": DEVELOPER_NAME}
...
def menu():
    ...
    return render_template("menu.html", menu=MENU, categories=categories, cart=cart, cfg=get_config())
...
def cart_page():
    ...
    return render_template("cart.html", cart=cart, total=total, table=table, cfg=get_config())
...
def add_to_cart():
    ...
    if existing:
        ...
        cart.append({"id": item["id"], "name": item["name"], "price": item["price"], "emoji": item["emoji"], "qty": 1})
    session["cart"] = cart
